...
export default function TodoDetailHTML(props: any) {
    ...
    return (
        <>
            {[...props.dateArray].includes(String(props.today)) && props.el?.status === true ? (
                <S.Wrapper onClick={MoveToPage(`/project/${props.el?.project.projectId}`)}>
                    <S.Color style={{ backgroundColor: props.el.project.projectColor }}></S.Color>
                    <S.Box style={{ backgroundColor: `${props.el.project.projectColor}40` }}>
                        <S.TitleBox>
